...
if [ -f /etc/nginx/http.d/panel.conf ]; then
  ...
  if [ -z $LE_EMAIL ]; then
    ...
    if [[ -f "/etc/nginx/certs/tls.key" && -f "/etc/nginx/certs/tls.crt" ]]; then
      # sed -i "s#ssl_certificate /etc/letsencrypt/live/<domain>/fullchain.pem;#ssl_certificate /etc/nginx/certs/tls.crt;#" /etc/nginx/http.d/panel.conf
      # sed -i "s#ssl_certificate_key /etc/letsencrypt/live/<domain>/privkey.pem;#ssl_certificate_key /etc/nginx/certs/tls.key;#" /etc/nginx/http.d/panel.conf
...
if [[ -f "/var/run/secrets/kubernetes.io/serviceaccount/token" ]]; then
  ...
  if [[ -n "$cluster_id" ]]; then
    ...
    if [[ -f "/etc/nginx/certs/tls.key" && -f "/etc/nginx/certs/tls.crt" ]]; then
      ...
      YAML_DATA=$(php artisan p:cluster:configuration $cluster_id --format=yaml |
        sed -e 's#^\(\s*cert:\s*\).*#\1/etc/kubectyl/certs/tls.cert#' \
