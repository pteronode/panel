...
class SnapshotAuthorizationTest extends ClientApiIntegrationTestCase
{
    ...
    public function testAccessToAServersBackupIsRestrictedProperly(string $method, string $endpoint)
    {
        ...
        $backup1 = Snapshot::factory()->create(['server_id' => $server1->id, 'completed_at' => CarbonImmutable::now()]);
        $backup2 = Snapshot::factory()->create(['server_id' => $server2->id, 'completed_at' => CarbonImmutable::now()]);
        $backup3 = Snapshot::factory()->create(['server_id' => $server3->id, 'completed_at' => CarbonImmutable::now()]);
        ...
        $this->actingAs($user)->json($method, $this->link($server1, '/snapshots/' . $backup1->uuid . $endpoint))
        ...
        $this->actingAs($user)->json($method, $this->link($server2, '/snapshots/' . $backup2->uuid . $endpoint))->assertForbidden();
        ...
        $this->actingAs($user)->json($method, $this->link($server1, '/snapshots/' . $backup2->uuid . $endpoint))->assertNotFound();
        $this->actingAs($user)->json($method, $this->link($server1, '/snapshots/' . $backup3->uuid . $endpoint))->assertNotFound();
        $this->actingAs($user)->json($method, $this->link($server2, '/snapshots/' . $backup3->uuid . $endpoint))->assertNotFound();
        $this->actingAs($user)->json($method, $this->link($server3, '/snapshots/' . $backup3->uuid . $endpoint))->assertNotFound();
    }
